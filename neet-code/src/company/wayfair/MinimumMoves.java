...
public class MinimumMoves {
    ...
    The maze is represented as an (nm) grid of cells, where each cell is either empty (denoted by 0), or contains an obstacle (denoted by 1). HackerMan is currently standing at cell (0, 0) and wishes to reach th cell (n-1, m-1).
    ...
    (i-x, j) where 1 ≤x≤k, provided cell (i - x, j) lies in the maze and there are no cells containing obstacles in the range (i, j) → (i - 1, j) → ... → (i - x, j).
    (i, j-x) where 1 ≤x≤k, provided cell (i, j - x) lies in the maze and there are no cells containing obstacles in the range (i, j) → (i, j - 1) → ... → (i, j - 1).
