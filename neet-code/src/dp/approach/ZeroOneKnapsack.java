package dp.approach;// Visit www.neon.rip for more content!
...
public class ZeroOneKnapsack {
    ...
    public static void main(String[] args) {
        List<Integer> profit = List.of(4, 4, 7, 1);
        List<Integer> weight = List.of(5, 2, 3, 1);
        int capacity = 8;

        System.out.println(optimizedDp(profit, weight, capacity));
        System.out.println(dp(profit, weight, capacity));
        System.out.println(dfs(profit, weight, capacity));
    }
    ...
    public static int dp(List<Integer> profit, List<Integer> weight, int capacity) {
        ...
        // items are in rows, capacity is in column,

        // Draw DP for better understanding for this example
        /*
                Capacity = 8
                profit = 4  4  7  1
                weight = 5  2  3  1
        * */
        ...
        for (int i = 1; i < N; i++) {
            for (int c = 1; c <= M; c++) {
                // skipping the current item means, we might or might not have picked the previous one
                int skip = dp[i - 1][c];
                int include = 0;
                if (c - weight.get(i) >= 0) {
