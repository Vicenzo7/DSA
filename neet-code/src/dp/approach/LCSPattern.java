...
public class LCSPattern {
    ...
    public static int longestCommonSubsequence(String text1, String text2) {
        ...
        System.out.println(memoization(0, 0, text1, text2, new HashMap<>()));
        ...
        System.out.println(memoizationDp(0, 0, text1, text2, cache));
    ...
    private static int memoizationDp(int i, int j, String text1, String text2, int[][] cache) {
        // if any of the text is out of bound return 0
        if (i == text1.length() || j == text2.length()) {
            return 0;
        }
        ...
        int ans;
        if (text1.charAt(i) == text2.charAt(j)) {
            ans = 1 + memoizationDp(i + 1, j + 1, text1, text2, cache);
        } else {
            int option1 = memoizationDp(i + 1, j, text1, text2, cache);
            int option2 = memoizationDp(i, j + 1, text1, text2, cache);
            ans = Math.max(option1, option2);
        }
